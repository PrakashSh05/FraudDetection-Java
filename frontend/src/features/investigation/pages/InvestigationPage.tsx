...
export const InvestigationPage = () => {
  ...
  const [searchId, setSearchId] = useState(transactionId || '1');

  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchId.trim()) {
      navigate(`/investigation/${searchId.trim()}`);
    }
  };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-3">
          ...
          <form onSubmit={handleSearchSubmit} className="flex-1 flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#E94F37]"
            />
            <Button type="submit" size="sm">Investigate ID</Button>
          </form>
        </div>
        ...
        <div className="p-8 text-center bg-rose-50 border border-rose-200 rounded-xl">
          ...
          <p className="text-sm font-semibold text-[#EF4444]">
            Transaction #{transactionId} not found in telemetry records.
          </p>
          <p className="text-xs text-gray-500 mt-1">
            Try searching for valid transaction IDs 1 to 50 generated by the seed dataset.
          </p>
          <div className="mt-4 flex justify-center gap-3">
            <Button size="sm" variant="outline" onClick={() => navigate('/investigation/1')}>
              Load Transaction #1
            </Button>
